...
def collate_fn_cuda(data):
    ...
    for x in batch:
        #if idx == 3:
        #    tup.append(torch.tensor(x).cuda() if len(x[0].size()) < 1 else pad_sequence(x, True, padding_value=-1).cuda())
        #else:
        tup.append(torch.tensor(x).cuda() if len(x[0].size()) < 1 else pad_sequence(x, True).cuda())
        #idx += 1
